...
void Photon::SetSCEta(float eta, float phi, float PV_x, float PV_y, float PV_z, bool isScEtaEB, bool isScEtaEE)
{
    ...
    if (isScEtaEB)
    {
        ...
        float angle_x0_y0 = 0;
        if (PV_x > 0)
            angle_x0_y0 = atan(PV_y / PV_x);
        else if (PV_x < 0)
            angle_x0_y0 = M_PI + atan(PV_y / PV_x);
        else if (PV_y > 0)
            angle_x0_y0 = M_PI_2;
        else
            angle_x0_y0 = M_PI_2;
...
void Photon::SetBIDBit(BooleanID id, bool idbit)
{
    switch (id)
    {
    case BooleanID::MVAIDWP80:
        j_mvaID_WP80 = idbit;
        break;
    case BooleanID::MVAIDWP90:
        j_mvaID_WP90 = idbit;
        break;
    default:
        break;
...
bool Photon::PassID(const TString ID) const
{
    ...
    // POG
    if (ID == "POGVeto")
        return CutBased() == WORKINGPOINT::VETO;
    if (ID == "POGLoose")
        return CutBased() == WORKINGPOINT::LOOSE;
    if (ID == "POGMedium")
        return CutBased() == WORKINGPOINT::MEDIUM;
    if (ID == "POGTight")
        return CutBased() == WORKINGPOINT::TIGHT;
    if (ID == "POGMVAIDWP80")
        return isMVAIDWP80();
    if (ID == "POGMVAIDWP90")
        return isMVAIDWP90();
