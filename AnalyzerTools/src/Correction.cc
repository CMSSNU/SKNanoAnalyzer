...
Correction::Correction(const TString &era, const TString &sample, const bool IsData, const string &btagging_eff_file, const string &ctagging_eff_file, const string &btagging_R_file, const string &ctagging_R_file)
{
    ...
    try{
        ...
        cerr << "[Correction::Correction] " << e.what() << endl;
    }
    ...
    EGM_keys["2022EE"] = "2022ReRecoE+PromptFG";
    ...
    JME_keys["2016preVFP"] = "Summer20UL16APV_JRV3_MC_######_AK4PFchs";
...
float Correction::GetTopPtReweight(const RVec<Gen> &gens) const{
    if(!Sample.Contains("TT") || !Sample.Contains("powheg")) return 1.;
    const Gen* top = nullptr;
    const Gen* antitop = nullptr;
    //loop while find top and antitop
    for(const auto &gen : gens){
        if(gen.PID() == 6 and gen.isHardProcess()) top = &gen;
        if(gen.PID() == -6 and gen.isHardProcess()) antitop = &gen;
        if(top != nullptr && antitop != nullptr) break;
    }
    if(top == nullptr || antitop == nullptr) return 1.;

    if(top->Pt() > 2000 || antitop->Pt() > 2000) return 1.;
